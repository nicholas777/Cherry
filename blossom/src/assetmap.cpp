...
namespace Cherry {
    ...
    std::filesystem::path Assetmap::Create(const std::filesystem::path &directory) {
        CH_PROFILE_FUNC();

        std::ofstream stream(directory.generic_u8string() + "/assetmap.yaml", std::fstream::out);
        stream.close();
    ...
    void Assetmap::Write(const std::filesystem::path &file) {
        YAML::Emitter out;
    ...
    void Assetmap::MapDirectory(const std::filesystem::path& dir) {
