...
namespace Cherry
{
	...
	public:
		...
		{
			return Vector3i(floor(color.x * 255),
							floor(color.y * 255),
							floor(color.z * 255));
		}
		...
		{
			return Vector4f(floor((float)color.x / 255),
							floor((float)color.y / 255),
							floor((float)color.z / 255),
							floor((float)color.w / 255));
		}
		...
		{
			return ((color.x & 0xff) << 24) + ((color.y & 0xff) << 16) + ((color.z & 0xff) << 8) + (color.w & 0xff);
		}
