...
namespace Cherry {
    ...
    static GLenum ToGLFormat(TextureWrap wrap) {
        switch (wrap) {
            ...
            default: CH_ASSERT(false, "Unknown Texture wrap"); return 0;
        }
    }
    ...
    static GLenum ToGLFormat(TextureFilter wrap) {
        switch (wrap) {
            ...
            default: CH_ASSERT(false, "Unknown Texture filter"); return 0;
        }
    }
    ...
    static GLenum ToGLFormat(TextureFormat wrap) {
        switch (wrap) {
            ...
            case TextureFormat::LuminanceWithAlpha: return GL_RG;
            case TextureFormat::Depth24Stencil8: return GL_DEPTH_STENCIL;
            default: CH_ASSERT(false, "Unknown Texture format"); return 0;
        }
    }
    ...
    OpenGLTexture::OpenGLTexture(std::string path, TextureParams params) {
        ...
        if (params.format == TextureFormat::Auto) {
            switch (channels) {
                ...
                default: CH_ASSERT(false, "Unknown texture format"); break;
            }
        } else {
    ...
    OpenGLTexture::OpenGLTexture(uint32_t width, uint32_t height, TextureParams params)
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, m_InternalFormat, width, height, 0, m_Format,
                     m_Format == GL_DEPTH_STENCIL ? GL_UNSIGNED_INT_24_8 : GL_UNSIGNED_BYTE,
                     nullptr);
    ...
    void OpenGLTexture::SetData(void* data) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, m_InternalFormat, m_Width, m_Height, 0, m_Format,
                     m_Format == GL_DEPTH_STENCIL ? GL_UNSIGNED_INT_24_8 : GL_UNSIGNED_BYTE, data);
    }
