...
namespace Cherry {
    ...
    void TransformationMatrix::Rotate(float rotation) {
        ...
        float val00 = rotCos * columns[0].x + -rotSin * columns[1].x;

        float val01 = rotCos * columns[0].y + -rotSin * columns[1].y;

        float val10 = rotSin * columns[0].x + rotCos * columns[1].x;

        float val11 = rotSin * columns[0].y + rotCos * columns[1].y;
    ...
    void Rotate(Matrix4x4f* mat, float rot) {
        ...
        float val00 = rotCos * (*mat)[0].x + -rotSin * (*mat)[1].x;

        float val01 = rotCos * (*mat)[0].y + -rotSin * (*mat)[1].y;

        float val10 = rotSin * (*mat)[0].x + rotCos * (*mat)[1].x;

        float val11 = rotSin * (*mat)[0].y + rotCos * (*mat)[1].y;
