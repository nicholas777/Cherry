...
namespace Cherry {
    ...
    public:
        ...
        const float& operator[](int index) const {
            CH_ASSERT(!(index > 1 || index < 0),
                      "specified index is either too large or too small");
        ...
        type operator+(const type& other) const { return type(x + other.x, y + other.y); }

        type operator+(const T& other) const { return type(x + other, y + other); }
    ...
    public:
        ...
        type operator-() const { return type(-x, -y, -z); }

        type operator+(const T& other) const { return type(x + other, y + other, z + other); }
    ...
    public:
        ...
        type operator-() const { return type(-x, -y, -z, -w); }
