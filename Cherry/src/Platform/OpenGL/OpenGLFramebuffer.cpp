	...
	void OpenGLFramebuffer::RegenerateFramebuffer()
	{
		...
		for (int i = 0; i < m_Attachments.size(); i++)
		{
			...
			if (IsDepthAttachment(attachment))
			{
				...
				glTextureStorage2D(m_AttachmentsGL[i], 1, GL_DEPTH24_STENCIL8, m_Data.width, m_Data.height);
			...
			else
			{
				...
				switch (attachment.format)
				{
					...
					case FramebufferTextureFormat::RedInteger:
						glTexImage2D(GL_TEXTURE_2D, 0, GL_R32I, m_Data.width, m_Data.height, 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE, nullptr);
		...
		{
			CH_ASSERT(m_ColorAttachments.size() <= 4, "Too many framebuffer color attachment, only four is supported");
